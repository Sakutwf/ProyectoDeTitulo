    ...
    <div class="content-wrapper">
      <div class="content-header">
        <div class="d-flex justify-content-between align-items-center">
          <h3 class="m-0"><i class="fa-solid fa-calendar-days me-2"></i>Gestión de Eventos</h3>
          <div class="d-flex">
            <input v-model="search" @input="onSearch" type="text" class="form-control form-control-sm me-2" placeholder="Buscar evento...">
            <button class="btn btn-danger btn-sm" @click="abrirModalNuevoEvento">
              <i class="fa-solid fa-calendar-plus me-1"></i> Nuevo Evento
            </button>
          </div>
        </div>
      </div>
              ...
              <table class="table custom-table">
                ...
                <tbody>
                  <tr v-for="(evento, i) in eventos" :key="evento.id">
                    ...
                    <td>{{ formatDate(evento.fecha_termino) }}</td>
                    <td>{{ evento.descripcion }}</td>
                    <td>
                      <span class="badge bg-cruz-roja">{{ evento.tipo }}</span>
                    </td>
                    <td>
                      <div class="d-flex justify-content-center">
                        <button @click="editEvento(evento.id)" class="btn btn-sm btn-outline-primary me-2" title="Editar">
                          <i class="fa-solid fa-edit"></i>
                        </button>
                        <button @click="eliminarEvento(evento.id)" class="btn btn-sm btn-outline-danger" title="Eliminar">
                  ...
                  <tr v-if="!eventos || eventos.length === 0">
                    <td colspan="7" class="text-center py-3">No hay eventos disponibles</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
async function fetchEventos(page = 1) {
  ...
  try {
    const res = await axios.get('http://localhost:8000/api/evento', { params })
...
function editEvento(id) {
  ...
  setTimeout(() => {
    if (eventoEditModal.value && typeof eventoEditModal.value.show === 'function') {
      eventoEditModal.value.show()
    }
  }, 100)
}
...
async function eliminarEvento(id) {
  ...
  if (confirm.isConfirmed) {
    try {
      await axios.delete(`http://localhost:8000/api/evento/${id}`)
      fetchEventos(meta.value.current_page)
